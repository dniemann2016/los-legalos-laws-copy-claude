...
import { Plus, Edit, Trash2, X, Link, ChevronRight, Network } from "lucide-react";
...
export default function RichterProfile() {
  ...
  const startEdit = (p) => {
    setForm({ name:p.name||"", gericht:p.gericht||"", kammer:p.kammer||"", rechtsgebiet:p.rechtsgebiet||"", kategorie:p.kategorie||"Richter", klaeger_rate:p.klaeger_rate||50, vergleich_rate:p.vergleich_rate||30, durchschnitt_dauer_monate:p.durchschnitt_dauer_monate||12, urteile_gesamt:p.urteile_gesamt||0, stil:p.stil||"Neutral", bekannt_fuer:p.bekannt_fuer||"", notizen:p.notizen||"" });
    setEditing(p.id);
    setShowForm(true);
  };
  ...
  const ProfileForm = () => (
    <div className="bg-[#fafafa] border border-[#f0f0f0] rounded-2xl p-5 space-y-4">
      <h3 className="text-sm font-semibold text-[#1a1a1a]">{editing ? "Profil bearbeiten" : "Neues Profil"}</h3>
      <div className="grid grid-cols-2 gap-3">
        <input className="border border-[#f0f0f0] rounded-xl px-3 py-2 text-sm bg-white col-span-2" placeholder="Name *" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} />
        <select className="border border-[#f0f0f0] rounded-xl px-3 py-2 text-sm bg-white" value={form.kategorie} onChange={e => setForm({ ...form, kategorie: e.target.value })}>
          {KATEGORIEN.map(k => <option key={k}>{k}</option>)}
        </select>
        <input className="border border-[#f0f0f0] rounded-xl px-3 py-2 text-sm bg-white" placeholder="Gericht / Kanzlei / Institution *" value={form.gericht} onChange={e => setForm({ ...form, gericht: e.target.value })} />
        <input className="border border-[#f0f0f0] rounded-xl px-3 py-2 text-sm bg-white" placeholder="Kammer / Senat" value={form.kammer} onChange={e => setForm({ ...form, kammer: e.target.value })} />
        <input className="border border-[#f0f0f0] rounded-xl px-3 py-2 text-sm bg-white" placeholder="Rechtsgebiet" value={form.rechtsgebiet} onChange={e => setForm({ ...form, rechtsgebiet: e.target.value })} />
        <select className="border border-[#f0f0f0] rounded-xl px-3 py-2 text-sm bg-white" value={form.stil} onChange={e => setForm({ ...form, stil: e.target.value })}>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[["Klägerquote %","klaeger_rate"],["Vergleichsrate %","vergleich_rate"],["Ø Dauer (Mo.)","durchschnitt_dauer_monate"],["Urteile gesamt","urteile_gesamt"]].map(([l,f]) => (
          <div key={f}>
            <label className="text-[10px] text-[#999] block mb-1">{l}</label>
            <input type="number" className="w-full border border-[#f0f0f0] rounded-xl px-2 py-2 text-xs bg-white" value={form[f]} onChange={e => setForm({ ...form, [f]: +e.target.value })} />
          </div>
        ))}
      </div>
      <input className="w-full border border-[#f0f0f0] rounded-xl px-3 py-2 text-sm bg-white" placeholder="Bekannt für..." value={form.bekannt_fuer} onChange={e => setForm({ ...form, bekannt_fuer: e.target.value })} />
      <textarea className="w-full border border-[#f0f0f0] rounded-xl px-3 py-2 text-xs bg-white" placeholder="Notizen" rows={2} value={form.notizen} onChange={e => setForm({ ...form, notizen: e.target.value })} />
      <div className="flex gap-2">
        <Button size="sm" onClick={save} className="bg-[#1a3560] text-white rounded-xl text-xs">{editing ? "Aktualisieren" : "Erstellen"}</Button>
      ...
      <div className="border-b border-[#f0f0f0] bg-white sticky top-0 z-20">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center gap-4">
          <div className="flex-1">
            <h1 className="text-sm font-bold text-[#1a1a1a]">Prozessbeteiligte</h1>
            <p className="text-[11px] text-[#666]">{profiles.length} Profile · Richter, Anwälte, Kanzleien & mehr</p>
          </div>
          <input placeholder="Suchen…" value={search} onChange={e => setSearch(e.target.value)}
            className="border border-[#f0f0f0] rounded-lg px-3 py-1.5 text-sm w-40 bg-[#fafafa] focus:outline-none focus:border-slate-300" />
          <button onClick={() => setShowNetzwerk(!showNetzwerk)}
            className={`flex items-center gap-1.5 text-xs font-semibold px-3 py-2 rounded-lg transition-colors border ${
              showNetzwerk ? "bg-[#1a3560] text-white border-[#1a3560]" : "border-[#f0f0f0] text-[#666] hover:border-slate-300"
            }`}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
          ...
          {["Alle", ...KATEGORIEN].map(k => (
            <button key={k} onClick={() => setKatFilter(k)}
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                katFilter === k ? "bg-[#1a3560] text-white border-[#1a3560]" : "border-[#f0f0f0] text-[#666] hover:border-slate-300"
              }`}>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            <p className="text-sm font-medium text-[#1a1a1a] mb-1">Noch keine Profile</p>
            <p className="text-xs text-[#999]">Klicken Sie auf „Profil" um ein Profil anzulegen</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map(p => (
              <div key={p.id} className="bg-white rounded-xl border border-[#f0f0f0] p-5 hover:border-slate-200 hover:shadow-sm transition-all">
                  ...
                  <>
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        <p className="text-xs text-[#666]">{p.gericht}{p.kammer ? ` · ${p.kammer}` : ""}</p>
                        {p.rechtsgebiet && <p className="text-[10px] text-[#999] mt-0.5">{p.rechtsgebiet}</p>}
                      </div>
                    ...
                    <div className="space-y-2.5 mb-4">
                      <div>
                        <p className="text-[10px] text-[#999] mb-1">Klägerquote</p>
                        <GaugeBar value={p.klaeger_rate || 0} color="bg-blue-500" />
                      </div>
                      <div>
                        <p className="text-[10px] text-[#999] mb-1">Vergleichsrate</p>
                        <GaugeBar value={p.vergleich_rate || 0} color="bg-violet-500" />
                      </div>
                    </div>
                    <div className="flex gap-4 text-xs text-[#666] mb-3 pt-3 border-t border-[#f0f0f0]">
                      {p.durchschnitt_dauer_monate > 0 && <span><strong className="text-[#1a1a1a]">{p.durchschnitt_dauer_monate}</strong> Mo. Ø</span>}
                      {p.urteile_gesamt > 0 && <span><strong className="text-[#1a1a1a]">{p.urteile_gesamt}</strong> Urteile</span>}
                      {(p.erfahrungen || []).length > 0 && <span><strong className="text-[#1a1a1a]">{p.erfahrungen.length}</strong> Erfahrungen</span>}
                    </div>
                    {p.bekannt_fuer && <p className="text-[11px] text-[#999] italic mb-3">"{p.bekannt_fuer}"</p>}
                    <div className="flex items-center justify-between">
                      <div>
                        {linkingId === p.id ? (
                          <div className="flex gap-2">
                            <select className="border border-[#f0f0f0] rounded-lg px-2 py-1.5 text-xs bg-[#fafafa]" value={selectedCase} onChange={e => setSelectedCase(e.target.value)}>
                            ...
                            <button onClick={() => { setLinkingId(null); setSelectedCase(""); }} className="text-xs text-[#999] hover:text-[#1a1a1a] px-1"><X className="w-3.5 h-3.5" /></button>
                          </div>
                        ) : (
                          <button onClick={() => setLinkingId(p.id)} className="flex items-center gap-1 text-[10px] text-[#999] hover:text-[#1a1a1a] transition-colors">
                    ...
                    {(() => {
                      ...
                      return linked.length > 0 ? (
                        <div className="mt-3 flex flex-wrap gap-1">
                          {linked.map(c => <span key={c.id} className="text-[10px] bg-[#fafafa] text-[#666] rounded-md px-2 py-0.5 border border-[#f0f0f0]">{c.fallname}</span>)}
                        </div>
                      ) : null;
                    })()}
